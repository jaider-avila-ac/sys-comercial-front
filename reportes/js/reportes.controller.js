...
function pct(a, b) { return (!b || b === 0) ? 0 : Math.min(100, Math.round((a / b) * 100)); }
...
const state = { page: 1, lineasRaw: [], lineasMeta: null, filtroLineas: "" };
const DASH = "—";

function skeletonKPIs() {
  ["kFacturado","kAplicado","kSaldoCierre","kDiferencia",
   "rFacturas","rSubtotal","rIva","rTotalFacturado",
   "pPagos","pRecibido","pAplicado",
   "pPagosCaja","pRecibidoCaja","pAplicadoCaja","pIngresosManuales","pEgresos","pBalanceReal",
...
function skeletonTables() {
  ...
  if (tM) tM.innerHTML = empty(8);
  ["mTotFacturado","mTotAplicado","mTotDiff","mTotIngManuales","mTotEgresos","mTotBalanceCaja"]
    .forEach(id => setText(id, DASH));
}
...
function renderLineas() {
  ...
  const rows = f
    ? state.lineasRaw.filter(r =>
        contains(r.factura_numero, f) || contains(r.item_nombre, f) ||
        contains(r.descripcion_manual, f) || contains(r.item_id, f))
    : state.lineasRaw;
...
function renderMensual(data = []) {
  ...
  let tf = 0, ta = 0, td = 0, tim = 0, teg = 0, tbc = 0;
  tbody.innerHTML = data.map(r => {
    const f  = Number(r.facturado || 0);
    const a  = Number(r.aplicado || 0);
    ...
    tf += f; ta += a; td += d; tim += im; teg += eg; tbc += bc;
    const cobertura = pct(a, f);
    const barColor  = cobertura >= 100 ? "var(--green)" : cobertura >= 60 ? "var(--amber)" : "var(--red)";
    ...
    return `<tr>
      ...
      <td class="text-num fw-semibold text-credit">${money(a)}</td>
...
async function loadKPIs() {
  ...
  try {
    ...
    // Libro de Recaudos (card izquierdo)
    setText("pPagos",    `${num(recaudos.pagos)} pagos`);
    setText("pRecibido", money(recaudos.total_caja));
    setText("pAplicado", money(recaudos.total_aplicado));
    // Balance Real de Caja (card derecho — IDs únicos)
    setText("pPagosCaja",        `${num(recaudos.pagos)} pagos`);
    setText("pRecibidoCaja",     money(recaudos.total_caja));
    setText("pAplicadoCaja",     money(recaudos.total_aplicado));
    setText("pIngresosManuales", money(recaudos.total_ingresos_manuales));
    ...
    const aplicado   = Number(recaudos.total_aplicado || 0);
    setText("kFacturado",   money(facturado));
    setText("kAplicado",    money(aplicado));
    setText("kSaldoCierre", money(cierreRow.saldo_al_cierre));
    setText("kDiferencia",  money(facturado - aplicado));
    setText("periodBadge",  `${fmtDate(desde)} — ${fmtDate(hasta)}`);
    setText("kCierreDate",  fmtDate(cierre));
    setStatus(`Reporte generado · ${num(ventas.facturas)} facturas emitidas entre ${fmtDate(desde)} y ${fmtDate(hasta)}.`, "ok");
  } catch (err) {
...
async function loadLineas(page = 1) {
  ...
  const hasta = document.getElementById("hasta").value;
  try {
    const res = await getVentasLineas({ desde, hasta, page });
    ...
    const tbody = document.getElementById("tbodyLineas");
    if (tbody) tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><i class="bi bi-exclamation-circle text-danger"></i><span>${err?.message || "Error cargando líneas"}</span></div></td></tr>`;
    setText("footLineasTotal", DASH);
    setText("lineasPagerInfo", DASH);
  }
}
...
async function loadMensual() {
  ...
  try {
    ...
    const tbody = document.getElementById("tbodyMensual");
    if (tbody) tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><i class="bi bi-exclamation-circle text-danger"></i><span>${err?.message || "Error cargando mensual"}</span></div></td></tr>`;
    ["mTotFacturado","mTotAplicado","mTotDiff","mTotIngManuales","mTotEgresos","mTotBalanceCaja"]
      .forEach(id => setText(id, DASH));
  }
}
...
(function init() {
  ...
  if (dDesde  && !dDesde.value)  dDesde.value  = monthStartISO();
  if (dHasta  && !dHasta.value)  dHasta.value  = monthEndISO();
  if (dCierre && !dCierre.value) dCierre.value = dHasta?.value || todayISO();
  skeletonKPIs();
  skeletonTables();
  loadKPIs();
  loadLineas(1);
